...
class ControllerTmblogCategory extends Controller {
	...
	public function index() {
		...
		$category_meta = $this->model_tmblog_category->getCategoryByName($filter_data);
		
		$data['articles'] = array();

		$results = $this->model_tmblog_category->getArticles($filter_data);
		$data['breadcrumbs'][] = array(
			'text' => $category_meta[0]['name'],
			'href' => $this->url->link('tmblog/category', 'tmblogcategory_id=' . $this->request->get['tmblogcategory_id'])
		);
		...
		$pagination->url = $this->url->link('tmblog/category','tmblogcategory_id=' . $this->request->get['tmblogcategory_id']. '&page={page}');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/tmblog/category.tpl')) { 
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/tmblog/category.tpl', $data));
		} else { 
			$this->response->setOutput($this->load->view('default/template/tmblog/category.tpl', $data));
		}
