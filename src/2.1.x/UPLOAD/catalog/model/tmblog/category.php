...
class ModelTmblogCategory extends Model {
	...
	public function categoryParentChildTree($parent_id = 0, $spacing = '', $category_tree_array = '') {
	      ...
	      $sql = $this->db->query("SELECT * FROM " . DB_PREFIX . "tmblogcategory mbc LEFT JOIN " . DB_PREFIX . "tmblogcategory_description mbcd ON(mbc.category_id=mbcd.category_id) LEFT JOIN " . DB_PREFIX . "tmblogcategory_to_store mbcs ON(mbc.category_id=mbcs.category_id) 
	...
	public function getTotalBlogCategories($data = array()){ 
		  ...
		  $sql = "SELECT COUNT(mb.blog_id) AS total FROM " . DB_PREFIX . "tmblog mb
			  LEFT JOIN " . DB_PREFIX . "tmblog_description mbd ON (mb.blog_id = mbd.blog_id) 
                          ...
                          LEFT JOIN " . DB_PREFIX . "tmblog_to_category mbc ON (mb.blog_id = mbc.blog_id)
	...
	public function getCategoryByName($data = array()){
		  $sql = "SELECT mbcd.name as name,mbcd.meta_title AS meta_title,mbcd.meta_description AS meta_description,mbcd.meta_keyword AS meta_keyword FROM " . DB_PREFIX . "tmblogcategory mbc LEFT JOIN " . DB_PREFIX . "tmblogcategory_description mbcd ON(mbc.category_id=mbcd.category_id)
	...
	public function getLatestposts($data = array()){
		...
		$sql = "SELECT DISTINCT(mbc.blog_id),mb.image,mb.publish_date,mbd.name,mbd.description  FROM " . DB_PREFIX . "tmblog mb
					   LEFT JOIN " . DB_PREFIX . "tmblog_description mbd ON (mb.blog_id = mbd.blog_id) 
                                           LEFT JOIN " . DB_PREFIX . "tmblog_to_store mb2s ON (mb.blog_id = mb2s.blog_id) 
					   LEFT JOIN " . DB_PREFIX . "tmblog_to_category mbc ON (mb.blog_id = mbc.blog_id)
