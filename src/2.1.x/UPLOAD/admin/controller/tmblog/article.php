...
class ControllerTmblogArticle extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_tmblog_article->editArticle($this->request->get['blog_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('tmblog/article', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $blog_id) {
				$this->model_tmblog_article->deleteArticle($blog_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('tmblog/article', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		
		$data['add'] = $this->url->link('tmblog/article/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('tmblog/article/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$data['articles'][] = array(
				...
				'edit'       => $this->url->link('tmblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $result['blog_id'] . $url, 'SSL')
		...
		$data['sort_name'] = $this->url->link('tmblog/article', 'token=' . $this->session->data['token'] . '&sort=md.name' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('tmblog/article', 'token=' . $this->session->data['token'] . '&sort=m.status' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('tmblog/article', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('tmblog/article.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['blog_id'])) {
			$data['action'] = $this->url->link('tmblog/article/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('tmblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $this->request->get['blog_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('tmblog/article', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$this->load->model('tmblog/article');

		if (isset($this->request->get['blog_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$article_info = $this->model_tmblog_article->getBlogarticle($this->request->get['blog_id']);
		}
		...
		if (isset($this->request->post['article_description'])) {
			...
			$data['article_description'] = $this->model_tmblog_article->getarticleDescriptions($this->request->get['blog_id']);
		} else {
		...
		if (isset($this->request->post['article_category'])) { 
			...
			$data['article_category'] = $this->model_tmblog_article->getBlogarticlecategory($this->request->get['blog_id']);
		...
		if (isset($this->request->post['article_store'])) { 
			...
			$data['article_store'] = $this->model_tmblog_article->getBlogarticleStores($this->request->get['blog_id']);
		...
		$this->response->setOutput($this->load->view('tmblog/article_form.tpl', $data));
	}
	public function add() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_tmblog_article->addArticle($this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
						
			$this->response->redirect($this->url->link('tmblog/article', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'tmblog/article')) {
