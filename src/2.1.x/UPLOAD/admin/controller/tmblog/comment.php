<?php
class ControllerTmblogComment extends Controller {

	private $error = array();

	public function index() { 
		$this->load->language('tmblog/comment');
		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('tmblog/comment');
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_tmblog_comment->editComment($this->request->get['comment_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $comment_id) {
				$this->model_tmblog_comment->deleteComment($comment_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		$data['add'] = $this->url->link('tmblog/comment/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('tmblog/comment/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$comment_total = $this->model_tmblog_comment->getTotalComments($filter_data);


		$results = $this->model_tmblog_comment->getArticleComments($filter_data);
		...
		foreach ($results as $result) {
			$data['articlecomments'][] = array(
				...
				'edit'       => $this->url->link('tmblog/comment/edit', 'token=' . $this->session->data['token'] . '&comment_id=' . $result['comment_id'] . $url, 'SSL')
		...
		$data['sort_articlename'] = $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . '&sort=md.name' . $url, 'SSL');
		$data['sort_user'] = $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . '&sort=mbc.user' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . '&sort=mbc.status' . $url, 'SSL');
		$data['sort_createdat'] = $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . '&sort=mbc.created_at' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('tmblog/comment.tpl', $data));
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['comment_id'])) {
			$data['action'] = $this->url->link('tmblog/comment/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('tmblog/comment/edit', 'token=' . $this->session->data['token'] . '&comment_id=' . $this->request->get['comment_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('tmblog/comment', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$this->load->model('tmblog/comment');

		if (isset($this->request->get['comment_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$comment_info = $this->model_tmblog_comment->getBlogcomment($this->request->get['comment_id']);
		}
		...
		/*$data['parentcategory'] = $this->model_tmblog_comment->getBlogall(0);
		...
		$this->response->setOutput($this->load->view('tmblog/comment_form.tpl', $data));
	}

	public function add() { 
		$this->language->load('tmblog/comment');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_tmblog_comment->addComment($this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
						
			$this->response->redirect($this->url->link('tmblog/comment', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'tmblog/comment')) {
	...
	public function autocomplete() {
			...
			if (isset($this->request->get['article_name'])) {
				...
				$results = $this->model_tmblog_article->getArticles($data);
